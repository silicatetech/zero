...
pub fn create_fat_filesystem(
    ...
    let fat_file = fs::OpenOptions::new()
        ...
        .open(out_fat_path)
        .unwrap();
    // Local patch: original `+ MB` (1 MiB) cushion is too small once
    // `needed_size` crosses ~1 GiB — `format_volume` returns ENOSPC because
    // the partition can't hold the FAT tables + the data. Generous padding
    // to be safe (a few hundred MiB on a 1.2 GiB image is fine).
    let fat_metadata_padding = core::cmp::max(256 * MB, needed_size / 8);
    let fat_size_padded_and_rounded = ((needed_size + 1024 * 64 - 1) / MB + 1) * MB + fat_metadata_padding;
    eprintln!(
        "[vendor-bootloader-patch] needed_size={} fat_partition_size={}",
        needed_size, fat_size_padded_and_rounded
    );
